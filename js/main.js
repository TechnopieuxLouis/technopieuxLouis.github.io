...
function showPage(id) {
    ...
    if (carouselResets[id]) carouselResets[id]();
}
...
document.querySelectorAll('.carousel-page').forEach(page => {
    ...
    // Touch swipe — on container to capture touches on any child element
    let startX = 0;
    let startY = 0;
    let isSwiping = false;
    ...
    container.addEventListener('touchmove', e => {
        ...
        // Horizontal swipe detected — mark as swiping
    ...
    // Ignored if the touch was actually a swipe
    ...
    carouselResets[page.id.replace('page-', '')] = () => goTo(0);
    goTo(0);
});
